...
function generateDashboardSummary(tickets) {
	...
	const statusSentence = `Of these, ${plural(accepted, 'ticket')} ${accepted !== 1 ? 'have' : 'has'} been accepted, ${plural(denied, 'ticket')} denied, and ${plural(pending, 'ticket')} ${pending !== 1 ? 'are' : 'is'} still pending${expired > 0 ? ` (${expired} of which ${expired !== 1 ? 'have' : 'has'} exceeded the response window)` : ''}.`;
