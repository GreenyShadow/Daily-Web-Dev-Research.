...
const queueSearchInput = qs('#queue-search');
if (queueSearchInput) {
	queueSearchInput.addEventListener('input', () => {
		queueSearchTerm = queueSearchInput.value.trim().toLowerCase();
		renderQueue();
	});
}

const workSearchInput = qs('#mywork-search');
if (workSearchInput) {
	workSearchInput.addEventListener('input', () => {
		workSearchTerm = workSearchInput.value.trim().toLowerCase();
		renderWorkList();
	});
}
...
function renderQueue(){
	...
	const queue = queueTickets()
		.filter(tk => matchesSearch(tk, queueSearchTerm))
		.slice()
		.sort((a, b) => {
	...
	board.innerHTML = '';
	if (queue.length === 0){
		board.innerHTML = `<div class="queue-empty">${queueSearchTerm ? 'No queued tickets match your search.' : 'The queue is empty — nothing waiting on a claim.'}</div>`;
		return;
	}
...
function renderWorkList(){
	...
	const filtered = mine.filter(tk => (currentFilter === 'all' || tk.status === currentFilter) && matchesSearch(tk, workSearchTerm));
	list.innerHTML = '';
	if (filtered.length === 0){
		list.innerHTML = `<div class="work-empty">${mine.length === 0 ? "Nothing assigned to you yet — claim something from the Queue." : 'No tickets match this view.'}</div>`;
		return;
	}
	filtered
		.slice()
