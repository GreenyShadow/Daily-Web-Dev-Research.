...
export default function AgentPage() {
  ...
  const queue = useMemo(() => tickets.filter((t) => t.status === 'accepted' && !t.assignedTo), [tickets]);
  const mine = useMemo(() => tickets.filter((t) => t.assignedTo === session.username), [tickets, session.username]);
  ...
  return (
    <Shell tabs={[...TABS.map((t) => (t.id === 'queue' ? { ...t, count: queue.length } : t.id === 'mywork' ? { ...t, count: mine.length } : t))]} activeTab={tab} onTabChange={setTab}>
      ...
      {tab === 'queue' && (
        <>
          ...
          <input
            ...
            style={{ marginBottom: 16, border: '1px solid var(--line)', borderRadius: 6, padding: '8px 11px', fontSize: 13, width: 280 }}
          />
      ...
      {selected && selected.assignedTo === session.username && ['in_progress', 'resolved'].includes(selected.status) && (
        <div style={{ position: 'fixed', bottom: 20, right: 20, zIndex: 120 }} className="card" >
          <div style={{ padding: 14, display: 'flex', gap: 8, alignItems: 'center' }}>
            <select value={reassignTarget} onChange={(e) => setReassignTarget(e.target.value)} style={{ border: '1px solid var(--line)', borderRadius: 6, padding: '6px 8px', fontSize: 13 }}>
              <option value="">Reassign to…</option>
              {agents.filter((a) => a.username !== session.username).map((a) => (
                <option key={a.username} value={a.username}>{a.username}</option>
              ))}
            </select>
            <button className="btn btn-ghost btn-sm" disabled={!reassignTarget || reassigning} onClick={() => handleReassign(selected.id)}>
