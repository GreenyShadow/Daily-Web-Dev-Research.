...
├── UI-React/               # React 19 + Vite front end — same features, same API, no backend changes
│   └── README.md            # setup, structure, and parity notes for the React client
└── docs/                    # Project documentation (this folder)
    ├── SRS.md               # Software Requirements Specification
    ├── BUGFIX_LOG.md
    ├── TEST_CASES.md
    ├── USER_GUIDE.md
    └── diagrams/            # use-case (per role), ERD, class, and architecture diagrams (SVG)
```
...
2. **Open the UI** — two options, both talk to the same API:
   - **Original (no build step):** open `UI/Auth/index.html` directly in a browser (or serve the `UI/` folder with any static file server, e.g. `npx serve UI`). The API has CORS enabled, so it works either way.
   - **React version:** `cd UI-React && npm install && npm run dev`, then open the URL Vite prints (`http://localhost:5173` by default). See `UI-React/README.md` for details.
