...
async function patchTicketApi(id, updates){
	...
	if (!res.ok) {
		...
		throw new Error(body.error || 'API update failed');
	}
	return res.json();
}
...
async function deleteTicketApi(id){
	...
	if (!res.ok) {
		const body = await res.json().catch(() => ({}));
		throw new Error(body.error || 'API delete failed');
	}
}
...
async function loadMyTickets(){
	...
	// Server uses `createdBy` while older/local copies used `name` — accept both.
	return all.filter(tk => ((tk.createdBy || tk.name) || '').trim().toLowerCase() === username.trim().toLowerCase());
}
...
const STAGE_LABELS = { accepted: 'Accepted', in_progress: 'In progress', resolved: 'Resolved', closed: 'Closed' };
...
// ── Dashboard ─────────────────────────────────────────────────────────────────
// "Accepted" covers the whole support pipeline (accepted, in_progress,
// resolved, closed) so a request the team has since resolved or closed still
// reads as accepted here, not as "awaiting review".
...
function matchesSearch(tk, term){
	if (!term) return true;
	const haystack = [tk.title, tk.description, tk.topic, tk.department, tk.id]
		.filter(Boolean)
		.join(' ')
		.toLowerCase();
...
// A member may only edit or cancel a request that hasn't been triaged yet.
function isUnprocessed(tk){
	return !tk.status || tk.status === 'pending';
}
...
function renderTicketList(tickets){
	...
	if (filtered.length === 0) {
		container.innerHTML = `<div class="ticket-empty">${tickets.length === 0 ? "You haven't submitted any tickets yet." : 'No tickets match this view.'}</div>`;
		return;
	}
...
function buildTicketCard(tk){
	...
	const statusLabel = status === 'accepted'
		? (STAGE_LABELS[tk.status] || 'Accepted')
		: { sent: 'Sent', denied: 'Denied', expired: 'Expired' }[status];
	...
	if (isUnprocessed(tk) && !usingFallback) {
		const actions = card.querySelector('.ticket-card-actions');
		const editBtn = document.createElement('button');
		editBtn.className = 'chip-btn chip-edit';
		editBtn.textContent = 'Edit';
		editBtn.addEventListener('click', () => openEditModal(tk));

		const cancelBtn = document.createElement('button');
		cancelBtn.className = 'chip-btn chip-cancel';
		cancelBtn.textContent = 'Cancel request';
		cancelBtn.addEventListener('click', () => cancelTicket(tk));
...
const editError = qs('#edit-error');

function openEditModal(tk){
	editError.classList.add('hidden');
	editError.textContent = '';
	qs('#edit-id').value = tk.id;
	qs('#edit-department').value = tk.department || 'Design';
	qs('#edit-priority').value = tk.priority || 'Low';
	qs('#edit-topic').value = tk.topic || '';
	qs('#edit-title').value = tk.title || '';
	qs('#edit-description').value = tk.description || '';
...
qs('#edit-modal-close').addEventListener('click', closeEditModal);
qs('#edit-cancel-close').addEventListener('click', closeEditModal);
...
editForm.addEventListener('submit', async (e) => {
	...
	const updates = {
		department: qs('#edit-department').value,
		priority: qs('#edit-priority').value,
		topic: qs('#edit-topic').value.trim(),
		title: qs('#edit-title').value.trim(),
		description: qs('#edit-description').value.trim(),
	};
	const saveBtn = qs('#edit-save-btn');
	saveBtn.disabled = true;
	try {
		await patchTicketApi(id, updates);
		closeEditModal();
...
async function cancelTicket(tk){
	if (!confirm(`Cancel the request "${tk.title}"? This can't be undone.`)) return;
	try {
		await deleteTicketApi(tk.id);
		await refreshTicketList();
		await refreshDashboard();
	} catch (err) {
